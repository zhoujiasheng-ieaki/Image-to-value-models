...
model_name='Img_to_value_model_4 (use h+c)'
...
class my_img_predict_value_model(nn.Module):
    def __init__(self,input_dim):
        ...
        '''self.convLSTM=My_enc_dec_convLSTM_model(input_dim=64,
                                                ...
                                                num_layers=1)'''
        self.convLSTM_Encoder=Encoder(input_dim=64, 
                             hidden_dim=64,
                             kernel_size=(3,3),
                             bias=True)
        ...
        self.conv128_64=nn.Conv2d(in_channels=128,
                             out_channels=64,
                             kernel_size=(3,3),
                             stride=1,
                             padding=1)
    ...
    def forward(self,input_):
        ...
        h_out, c_out = self.convLSTM_Encoder(x) #h_out:(batch,channels,h,w)
        out=torch.cat([h_out,c_out],axis=1)
